...
- [x] `app/main.go` собирает runtime из доменных интерфейсов, а не из одной связки прямых вызовов.
- [x] Один и тот же spam case проходит end-to-end через internal queue без изменения пользовательского поведения.
