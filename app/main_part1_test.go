...
func TestMakeSpamLogger_SaveAudit(t *testing.T) {
	...
	assert.InDelta(t, 2.0, savedMsgs[0].Score, 1e-9)
...
func Test_makeDetector(t *testing.T) {
	...
	t.Run("uses rule set values", func(t *testing.T) {
		...
		assert.InDelta(t, 0.5, res.AbnormalSpacing.ShortWordRatioThreshold, 1e-9)
		assert.InDelta(t, 0.2, res.AbnormalSpacing.SpaceRatioThreshold, 1e-9)
...
func TestBootstrapRuleSet(t *testing.T) {
	...
	assert.InDelta(t, 0.4, got.AbnormalSpacing.SpaceRatioThreshold, 1e-9)
