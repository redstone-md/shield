...
CREATE TABLE IF NOT EXISTS workspace_members (
	...
	UNIQUE(gid, workspace_id, user_id)
)`,
...
CREATE TABLE IF NOT EXISTS workspace_members (
	...
	AddSame(CmdAddWorkspace, `INSERT INTO workspaces (gid, name, owner_id, status, created_at, updated_at)
	VALUES (?, ?, ?, ?, ?, ?)`).
	AddSame(CmdGetWorkspace, `SELECT id, gid, name, owner_id, status, created_at, updated_at
	FROM workspaces WHERE gid = ? AND name = ?`).
	AddSame(CmdListWorkspaces, `SELECT id, gid, name, owner_id, status, created_at, updated_at
	FROM workspaces WHERE gid = ? ORDER BY created_at ASC`).
	AddSame(CmdUpdateWorkspace, `UPDATE workspaces SET name = ?, status = ?, updated_at = ?
	WHERE gid = ? AND id = ?`).
	AddSame(CmdAddWorkspaceMember, `INSERT INTO workspace_members (gid, workspace_id, user_id, role, granted_at)
	VALUES (?, ?, ?, ?, ?)`).
	AddSame(CmdGetWorkspaceMember, `SELECT id, gid, workspace_id, user_id, role, granted_at
	FROM workspace_members WHERE gid = ? AND workspace_id = ? AND user_id = ?`).
	AddSame(CmdListWorkspaceMembers, `SELECT id, gid, workspace_id, user_id, role, granted_at
	FROM workspace_members WHERE gid = ? AND workspace_id = ? ORDER BY granted_at ASC`).
	AddSame(CmdUpdateWorkspaceMemberRole, `UPDATE workspace_members SET role = ? WHERE gid = ? AND workspace_id = ? AND user_id = ?`).
	AddSame(CmdRemoveWorkspaceMember, `DELETE FROM workspace_members WHERE gid = ? AND workspace_id = ? AND user_id = ?`)
...
func (w *Workspaces) Add(ctx context.Context, rec WorkspaceRecord) (int64, error) {
	...
	now := time.Now().UTC()
	result, err := w.ExecContext(ctx, query, w.GID(), rec.Name, rec.OwnerID, rec.Status, now, now)
	if err != nil {
...
func (w *Workspaces) Get(ctx context.Context, name string) (WorkspaceRecord, error) {
	...
	if err := w.GetContext(ctx, &rec, query, w.GID(), name); err != nil {
...
func (w *Workspaces) List(ctx context.Context) ([]WorkspaceRecord, error) {
	...
	if err := w.SelectContext(ctx, &recs, query, w.GID()); err != nil {
...
func (w *Workspaces) Update(ctx context.Context, id int64, name, status string) error {
	...
	if _, err := w.ExecContext(ctx, query, name, status, time.Now().UTC(), w.GID(), id); err != nil {
...
func (w *Workspaces) AddMember(ctx context.Context, workspaceID int64, userID, role string) error {
	...
	if _, err := w.ExecContext(ctx, query, w.GID(), workspaceID, userID, role, time.Now().UTC()); err != nil {
...
func (w *Workspaces) GetMember(ctx context.Context, workspaceID int64, userID string) (WorkspaceMemberRecord, error) {
	...
	if err := w.GetContext(ctx, &rec, query, w.GID(), workspaceID, userID); err != nil {
		if err == sql.ErrNoRows {
...
func (w *Workspaces) ListMembers(ctx context.Context, workspaceID int64) ([]WorkspaceMemberRecord, error) {
	...
	if err := w.SelectContext(ctx, &recs, query, w.GID(), workspaceID); err != nil {
...
func (w *Workspaces) UpdateMemberRole(ctx context.Context, workspaceID int64, userID, role string) error {
	...
	if _, err := w.ExecContext(ctx, query, role, w.GID(), workspaceID, userID); err != nil {
...
func (w *Workspaces) RemoveMember(ctx context.Context, workspaceID int64, userID string) error {
	...
	if _, err := w.ExecContext(ctx, query, w.GID(), workspaceID, userID); err != nil {
